...
export const ClickMenu: MemoExoticComponent<
...
> = React.memo(
  ...
  >((props: Partial<MpDomProps>, ref) => {
    ...
    const cnPrefix =
      language === "cn" ? ICON.Ok : ICON.CheckboxBlankCircleOutline
    const enPrefix =
      language === "en" ? ICON.Ok : ICON.CheckboxBlankCircleOutline
    const mode = useSelector(modeSelector)
    const darkPrefix =
      mode === "dark" ? ICON.Ok : ICON.CheckboxBlankCircleOutline
    const lightPrefix =
      mode === "light" ? ICON.Ok : ICON.CheckboxBlankCircleOutline

    const uiName = useSelector(uiNameSelector)
    const oscPrefix =
      uiName === "osc" ? ICON.Ok : ICON.CheckboxBlankCircleOutline
    const uoscPrefix =
      ...
      const commonItems: MenuItem[] = [
        ...
        },
        {
          ...
          children: speedList.map((i) => {
            const prefix =
              i === speed ? ICON.Ok : ICON.CheckboxBlankCircleOutline
            return {
